...
// (MQTT, NATS, HTTP, CoAP) cuando se activan las reglas.
package edge
...
import (
	...
	"github.com/cbiale/sensorwave/middleware/cliente_nats"
)
...
// RegistrarEjecutorNATS conecta a un servidor NATS, crea un ejecutor y lo registra
// en el motor de reglas.
//
// Retorna el cliente para que el llamador pueda hacer defer cliente.Desconectar()
func RegistrarEjecutorNATS(motor *MotorReglas, nombre, direccion, puerto string) (middleware.Cliente, error) {
	if motor == nil {
		return nil, fmt.Errorf("motor de reglas no puede ser nil")
	}

	cliente, err := cliente_nats.Conectar(direccion, puerto)
	if err != nil {
		return nil, fmt.Errorf("no se pudo conectar a NATS en %s:%s: %v", direccion, puerto, err)
	}
	...
	log.Printf("Ejecutor NATS '%s' registrado (servidor: %s:%s)", nombre, direccion, puerto)
	return cliente, nil
}
